...
<div class="flex flex-row bg-white border-2 border-gray-400 rounded-xl w-[40rem] h-[3.75rem] items-center focus-within:border-orange-500">
	...
	<input bind:value={query} on:keydown={handleKeydown} class="outline-none caret-transparent focus:caret-black w-full text-xl" placeholder="Enter where you want to go" />
	<div class="relative group">
		<button class="mx-4 flex flex-row items-center dropdown">
		...
		<div class="absolute p-3 w-[10rem] bg-white rounded-lg shadow-xl dropdown-content hidden group-hover:flex flex-col items-start">
			...
			<button on:click={()=> condition = 'location'} type="button" class="hover:bg-[#f9f9f9] w-full text-start">Location</button>
		</div>
	</div>
